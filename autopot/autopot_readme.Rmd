---
title: "Automatically Selecting Thresholds"
...
A smooth estimation of the number of peaks per year (PPY) is obtained using isotonic regression due to fluctuations in the PPY, which is created by the declustering method.
...
This corresponds to the column `auto`.
...
Table 1 indicates that the proportions of thresholds with specific PPY according to its super region. We see that super regions 1, 3 and 4 have a higher percentage of sites where the automatic threshold was selected below 1.5 PPY, which can be explained by a majority of sites where the flood regime is dominated by the spring freshet.
...
xthresh <- read.csv(gzfile('autopot.csv.gz'))
